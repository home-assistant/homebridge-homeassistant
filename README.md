...
For binary sensors with the `gas` `device_class` you can also set `homebridge_gas_type` to `co` or `co2` to control how the entity is shown in Homebridge (`co` is default).
